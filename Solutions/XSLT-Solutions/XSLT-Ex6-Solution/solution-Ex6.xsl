                ...
                <xsl:apply-templates select="$dickinsonColl//body" mode="toc">
                    <xsl:sort>
                     <xsl:variable name="apos">'</xsl:variable>
                        <xsl:value-of select="translate((descendant::l[1]), $apos, '')"/>
                    </xsl:sort>
                    <!--ebb: We are sorting the poems alphabetically by the first line (in the first stanza): //lg[1]/l[1]. 
                        We also did the OPTIONAL CHALLENGE to translate away the apostrophe, so poems starting with 'Tis or 'Twas get sorted with the letter T.
                       To do this: we open up the <xsl:sort> element to introduce a variable and a calculation with <xsl:value-of> so that we can translate the apostrophe out of our sort.
                       The apostrophe doesn't go away; it just is translated during the sort to be no character at all, so the sorting is based on the next character. -->
                </xsl:apply-templates>
            </ul>
          <hr/></div>
          <div id="main">
    ...
    <xsl:template match="body" mode="toc">
       <li><a href="#p{//idno}">
           ...
           <!--ebb: We used an Attribute Value Template (AVT) here to construct the @href that our internal links need to target so we can jump to each poem. Notice, the <idno> element is up in the teiHeader and not inside the body element, so this is one case where we actually DO want to begin our XPath from the document node. 
               We used nearly the same AVT in the next template rule with the mode set to output full poems, in order to generate @id attributes that serve as the targets for our links. 
               The difference here is that we need a hashtag (#) to start the @href that points to the @id (which has no hashtag) in order to make the internal links work to keep us jumping on the same HTML page. 
               (Without the hashtag, a web browser thinks these links refer to new files that it won't be able to find.)-->
           <strong><xsl:apply-templates select="descendant::title"/></strong></a>: 
           <xsl:apply-templates select="lg[1]/l[1]"/><xsl:text> [Variant Passages: </xsl:text><xsl:value-of select="count(descendant::app)"/><xsl:text>]</xsl:text>
       </li>
    </xsl:template>
